...
func (db *Repo) GetBooksByUser(userID uuid.UUID) ([]Book, error) {
	query := `
				SELECT id, title, author_id, genre, year_public, year_read, grade, description
				FROM rate_books.books
				WHERE owner_id = $1
	...
	if err != nil {
		return nil, fmt.Errorf("error in GetBooksByUser query: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("error in GetBooksByUser scan: %w", err)
		}
...
// получение списка всех книг в системе
func (db *Repo) GetAllBooks() ([]Book, error) {
	query := `
				SELECT id, title, author_id, genre, year_public, year_read, grade, description
				FROM rate_books.books
			`
